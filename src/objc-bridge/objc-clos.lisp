...
(defparameter *objc-import-private-ivars* t
  "When true, the CLASS-DIRECT-SLOTS of imported ObjC classes will
contain slot definitions for instance variables whose name starts with
an underscore.  Note that this may exacerbate compatibility
problems.")
...
(defun recognize-objc-object (p)
  (labels ((recognize (p mapped)
             (let* ((idx (objc-class-id p)))
               (if idx
                   (ccl::%set-macptr-type p (dpb objc-flag-class objc-type-flags idx))
                   (if (setf idx (objc-metaclass-id p))
                       (ccl::%set-macptr-type p (dpb objc-flag-metaclass objc-type-flags idx))
                       (if (setf idx (%objc-instance-class-index p))
                           (ccl::%set-macptr-type p idx)
                           (unless mapped
...
(defun %objc-domain-instance-class-wrapper (p)
  (let* ((type (ccl::%macptr-type p))
    ...
    (ecase flags
      ...
      (#.objc-flag-metaclass (ccl::%class.own-wrapper *objc-metaclass-class*)))))
...
#+ccl
(ccl:defloadvar *objc-object-domain*
    (ccl::register-foreign-object-domain :objc
...
(defun %set-objc-instance-type (p)
  (unless (ccl:%null-ptr-p p)
    (let* ((parent (ccl:pref p #-gnu-objc :objc_object.isa #+gnu-objc :objc_object.class_pointer))
           (id (objc-class-id parent)))
      (when id
        (ccl::%set-macptr-domain p *objc-object-domain*)
        (ccl::%set-macptr-type p id)))))
...
(defparameter *objc-metaclass-class* (find-class 'objc:objc-metaclass))

(defclass objc:objc-class (objc:objc-class-object)
  ())

(defparameter *objc-class-class* (find-class 'objc:objc-class))
      ...
      (print-unreadable-object (o stream :type t)
        (format stream
                (if (and (typep o 'ns::ns-string)
                    ...
                    "~s (#x~x)"
                    "~a (#x~x)")
                (nsobject-description o) (ccl:%ptr-to-int o)))
      (format stream "#<Bogus ObjC Object #x~X>" (ccl:%ptr-to-int o))))
...
(defun make-objc-class-object-slots-vector (class meta)
  #-ccl (niy make-objc-class-object-slots-vector class meta)
  #+ccl
  (let* ((n (1+ (length (ccl::extract-instance-effective-slotds meta))))
         (slots (ccl::allocate-typed-vector :slot-vector n (ccl::%slot-unbound-marker))))
    (setf (ccl::slot-vector.instance slots) class)
    slots))
...
(defmethod slot-boundp-using-class ((class objc:objc-class-object)
                                    instance
                                    (slotd closer-mop:standard-effective-slot-definition))
  (ccl::%std-slot-vector-boundp (%objc-domain-slots-vector instance) slotd))

(defmethod slot-value-using-class ((class objc:objc-class-object)
                                   instance
                                   (slotd closer-mop:standard-effective-slot-definition))
  (ccl::%std-slot-vector-value (%objc-domain-slots-vector instance) slotd))
...
(defmethod (setf slot-value-using-class)
    (new
     ...
     instance
     (slotd closer-mop:standard-effective-slot-definition))
  (ccl::%set-std-slot-vector-value (%objc-domain-slots-vector instance) slotd new))
...
(defmethod shared-initialize :after ((slotd foreign-direct-slot-definition)
  ...
  (unless (typep foreign-type 'foreign-type)
    (setf foreign-type (ccl::parse-foreign-type foreign-type)))
...
(defun set-objc-foreign-direct-slot-offsets (dslotds bit-offset)
  (dolist (d dslotds)
    (let* ((ftype (foreign-slot-definition-foreign-type d))
      ...
      (setf bit-offset (+ bit-offset (foreign-type-bits ftype))))))
  ...
  (dolist (dslotd dslotds)
    (when (typep dslotd 'foreign-direct-slot-definition)
      (let* ((string (lisp-defined-slot-name-to-objc-slot-name (slot-definition-name dslotd)))
        ...
        (ccl:with-cstrs ((name string)
                     (encoding encoding))
          (when (eql *NO* (#_class_addIvar class name size align encoding))
            (error "class_addIvar failed"))
          (ccl:with-macptrs ((ivar (#_class_getInstanceVariable class name)))
            (unless (ccl:%null-ptr-p ivar)
...
(defun %make-objc-ivars (class)
  (let* ((start-offset (superclass-instance-size class))
    ...
    (if (null foreign-dslotds)
        (values (ccl:%null-ptr) start-offset)
        (let* ((n (length foreign-dslotds))
               (offset start-offset)
               (ivars (malloc (+ 4 (* n (ccl::%foreign-type-or-record-size
                                         :objc_ivar :bytes))))))
          (setf (ccl:pref ivars :objc_ivar_list.ivar_count) n)
          (do* ((l foreign-dslotds (cdr l))
                (dslotd (car l) (car l))
                (ivar (ccl:pref ivars :objc_ivar_list.ivar_list)
                      (ccl:%inc-ptr ivar (ccl::%foreign-type-or-record-size
                                      :objc_ivar :bytes))))
            ...
            (let* ((string (lisp-defined-slot-name-to-objc-slot-name (slot-definition-name dslotd)))
                   ...
                   (encoding (progn
                               (ensure-foreign-type-bits type)
                               (encode-objc-type type))))
              (setf offset (foreign-direct-slot-definition-bit-offset dslotd))
              (setf (ccl:pref ivar :objc_ivar.ivar_name) (make-cstring string)
                    (ccl:pref ivar :objc_ivar.ivar_type) (make-cstring encoding)
                    (ccl:pref ivar :objc_ivar.ivar_offset) (ash offset -3))
...
(defun %objc-ivar-offset-in-class (name c)
  ...
  ;; NIL.
  #-ccl (niy %objc-ivar-offset-in-class name c)
  #+ccl (progn
          #+(or apple-objc-2.0 cocotron-objc)
          (ccl:with-cstrs ((name name))
            (ccl:with-macptrs ((ivar (#_class_getInstanceVariable c name)))
              (unless (ccl:%null-ptr-p ivar)
                (#_ivar_getOffset ivar))))
          #-(or apple-objc-2.0 cocotron-objc)
          (when (objc-class-p c)
            (ccl:with-macptrs ((ivars (ccl:pref c :objc_class.ivars)))
              (unless (ccl:%null-ptr-p ivars)
                (loop
  ...
  (labels ((locate-objc-slot (name class)
             (unless (ccl:%null-ptr-p class)
               (or (%objc-ivar-offset-in-class name class)
                   (ccl:with-macptrs ((super #+(or apple-objc-2.0 cocotron-objc)
                                         (#_class_getSuperclass class)
                                         #-(or apple-objc-2.0 cocotron-objc)
                                         (ccl:pref class :objc_class.super_class)))
                     (unless (or (ccl:%null-ptr-p super) (eql super class))
...
(defun compute-foreign-slot-accessors (eslotd)
  (let* ((ftype (foreign-slot-definition-foreign-type eslotd))
         (ordinal (ccl::foreign-type-ordinal ftype)))
    (etypecase ftype
         ...
         (if (= bits align)
             (case bits
               ...
               (t (values (lambda (ptr offset)
                              (ccl::%get-bitfield ptr offset bits))
                          (lambda (ptr offset new)
                              (setf (ccl::%get-bitfield ptr offset bits) new)))))
      ...
      (foreign-pointer-type
       (if (objc-id-type-p ftype)
           (values #'ccl:%get-ptr #'ccl::%set-ptr)
           (let* ((to (foreign-pointer-type-to ftype))
                  (to-ordinal (if to (ccl::foreign-type-ordinal to) 0)))
             (values (lambda (ptr offset)
                         (let* ((p (ccl:%null-ptr)))
                           (ccl:%setf-macptr p (ccl:%get-ptr ptr offset))
                           (unless (ccl:%null-ptr-p p)
                             (ccl::%set-macptr-domain p 1)
                             (ccl::%set-macptr-type p to-ordinal))
                           p))
                     #'ccl::%set-ptr))))
      (foreign-mem-block-type
       (let* ((nbytes (ccl::%foreign-type-or-record-size ftype :bytes)))
         (values (lambda (ptr offset)
                     (let* ((p (ccl:%inc-ptr ptr offset)))
                       (ccl::%set-macptr-type p ordinal)
                       p))
                 (lambda (pointer offset new)
                     (setf (ccl::%composite-pointer-ref
                            nbytes
                            pointer
                            offset)
                           new))))))))
        ...
        (let* ((initer (dolist (s direct-slots)
               ...
               (eslotd
                (make-effective-slot-definition
                 ...
                 :name name
                 :allocation :instance
                 :type (or (ccl::%slot-definition-type first) t)
                 ...
                 :initfunction (if initer
                                   (ccl::%slot-definition-initfunction initer))
  ...
  (flet ((foreign-slot-p (s) (typep s 'foreign-effective-slot-definition)))
    (let* ((cpl (ccl::%class-precedence-list class))
      ...
      (loop for islot in instance-slots
         ...
         do (setf (ccl::%slot-definition-location islot) loc))
      (dolist (cslot class-slots)
        (setf (ccl::%slot-definition-location cslot)
              (assoc (ccl::%slot-definition-name cslot)
                     (ccl::%class-get (ccl::%slot-definition-class cslot) :class-slots)
                     :test #'eq)))
      (dolist (fslot foreign-slots)
        (setf (ccl::%slot-definition-location fslot)
              (determine-foreign-slot-location
               class
               (ccl::%slot-definition-name fslot))))
...
(defun create-foreign-instance-slot-vector (class)
  (let* ((max 0))
    (dolist (slotd (class-slots class)
             (unless (zerop max)
               (ccl::allocate-typed-vector :slot-vector (1+ max) (ccl::%slot-unbound-marker))))
      (when (typep slotd 'closer-mop:standard-effective-slot-definition)
                  ...
                  (when (and (or (eq slot-names t) 
                    ...
                    (let ((newval (funcall initfunction)))
                      (unless (or (null typepred)
                                  (funcall typepred newval))
                        (report-bad-arg newval slot-type))
                      (setf (ccl::%standard-instance-instance-location-access
                             instance loc)
                            newval)))))))))
    instance))
    ...
    (let* ((existing-metaclass (class-of class)))
      (if (and (eq (class-of metaclass)
               ...
               (ccl:with-macptrs ((super #+(or apple-objc-2.0 cocotron-objc)
                                     ...
                                     #-(or apple-objc-2.0 cocotron-objc)
                                     (ccl:pref metaclass :objc_class.super_class)))
                 (and (not (ccl:%null-ptr-p super))
                      (not (%objc-metaclass-p super))
                      (ccl:%null-ptr-p
                       #+(or apple-objc-2.0 cocotron-objc)
                       (#_class_getSuperclass super)
                       #-(or apple-objc-2.0 cocotron-objc)
                       (ccl:pref super :objc_class.super_class)))))
...
(defmethod shared-initialize ((class objc:objc-class-object) slot-names &rest initargs)
  (ccl::%shared-initialize class slot-names initargs))
  ...
  (or
   (let* ((type nil))
     (if (and (symbolp instance)
              (subtypep (setf type (cdr (assq 'type (nth-value 2 (variable-information instance env)))))
                        'objc:objc-object
                        env)
              (consp slot-name)
              ...
              (setf slot-name (cadadr slot-name))
              (quoted-form-p slot-name)
              (setf slot-name (cadr slot-name)))
