...
(defmethod save ((self C-compiled-patch))
  (let* ((my-in-types (get (pw-function self) '*type-intypes*))
         ...
         (comp-name  
          (choose-new-file-dialog :directory (format () "~A.comp" (string-downcase (pw-function self))) 
                                  :button-string "Save patch as"))
         (*print-pretty* nil))
     (multiple-value-bind (new-name exists?)
...
A previous function called ~D will be redefined" new-name new-name)
    ...
    (with-cursor *watch-cursor*
      (WITH-OPEN-FILE (out comp-name :direction :output 
