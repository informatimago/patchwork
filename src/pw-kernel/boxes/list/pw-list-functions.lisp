...
(pw:defunt first ((list list)) nil)
(pw:defunt rest ((list list)) list)
(pw:defunt butlast ((list list) &optional (num fix>=0 (:value 1))) list)
(pw:defunt reverse ((sequence list)) list)
(pw:defunt length ((sequence list)) fix)
(pw:defunt list (&rest (arguments nilNum)) list)
(pw:defunt mapcar ((function symbol (:value "1+")) (list list)
                &rest (more-lists list)) list)
(pw:defunt apply ((function symbol (:value "+")) (first-arg list)
               &rest (more-args list (:type-list ()))) nil)
(pw:defunt cons ((x nilnum) (list-or-thing list (:type-list ()))) list)
(pw:defunt funcall ((function symbol (:value "+"))
                    &rest (arguments nilnum)) nil)
(pw::defunt print ((object nilnum) 
                   &optional (output-stream list (:value '() :type-list ()))) nil)

(in-package :pw)
(eval-when (load eval compile)
  (import '(epw::make-list2)))
