...
(defun write-midi-note (dur chan key vel) 
  (unless (or (minusp key) (> key 127))
    (setq chan (1- chan))
    (setf dur (* 10 dur))
    (let ((event (patchwork.midi:MidiNewEv patchwork.midi:typeNote)))	
      (unless (patchwork.midi:null-event-p event)	
        (patchwork.midi:chan event chan)			
        (patchwork.midi:port event 0)			
        (patchwork.midi:field event 0 key)		
        (patchwork.midi:field event 1 vel)		
        (patchwork.midi:field event 2 dur)
        (patchwork.midi:midi-write  event)))))
...
(defun write-pitch-bend-value (chan value &optional (ls 0))
  (setq chan (1- chan))
  (let ((event (patchwork.midi:MidiNewEv patchwork.midi:typePitchWheel)))	
    (unless (patchwork.midi:null-event-p event)	
      (patchwork.midi:chan event chan)			
      (patchwork.midi:port event 0)			
      (patchwork.midi:field event 0 ls)		
      (patchwork.midi:field event 1 value)		
      (patchwork.midi:MidiSendIm patchwork.midi:*pw-refnum* event))))

(defun write-controller-value (chan controller value)
  (setq chan (1- chan))
  (let ((event (patchwork.midi:MidiNewEv patchwork.midi:typeCtrlChange)))	
    (unless (patchwork.midi:null-event-p event)	
      (patchwork.midi:chan event chan)			
      (patchwork.midi:port event 0)			
      (patchwork.midi:field event 0 controller)		
      (patchwork.midi:field event 1 value)		
      (patchwork.midi:MidiSendIm patchwork.midi:*pw-refnum* event))))
...
(defun write-program-change-value (chan program) 
  ...
  (let ((event (patchwork.midi:MidiNewEv patchwork.midi:typeProgChange)))	
    (unless (patchwork.midi:null-event-p event)	
      (patchwork.midi:chan event chan)			
      (patchwork.midi:port event 0)			
      (patchwork.midi:field event 0 program)		
      (patchwork.midi:MidiSendIm patchwork.midi:*pw-refnum* event))))


(defun write-pressure-value (chan key value)
  (setq chan (1- chan))
  (let ((event (patchwork.midi:MidiNewEv patchwork.midi:typeKeyPress)))	
    (unless (patchwork.midi:null-event-p event)	
      (patchwork.midi:chan event chan)			
      (patchwork.midi:port event 0)			
      (patchwork.midi:field event 0 key)		
...
(defun read-from-midi ()
  (let ((res)(temp)(start-time)  res2)
    (when (car (setq temp (multiple-value-list (patchwork.midi:midi-read))))
